...
/**
 * Interface van htmlLawed
 * @see htmlLawed
 * @package WordPressSanitizer
 */
interface iHtmlLawed {
	static function purify($t, $C = 1, $S = array());

	static function hl_tidy_callback($var);

	static function hl_cmtcd($t);

	static function hl_ent($t);

	static function hl_prot($p, $c = null);

	static function hl_tag($t);

	static function hl_version();
}
 ...
 * Script adapted to static class by Alex Pot of www.smartscripts.nl
 * Added methods:
 * @see htmlLawed::_element_get_tags_and_content()
 * @see htmlLawed::_head_format()
 * @see htmlLawed::_blocklevel_spacers()
 * @see htmlLawed::_html_corrections()
 * @see htmlLawed::hl_tidy_callback()
 *
 * @package WordPressSanitizer
*/
class htmlLawed implements iHtmlLawed {
	...
	private static $_C = Array(
		...
		"base_url" => "",
		"cdata" => 3,
		"clean_ms_char" => 0, //waarde 2 zorgde ervoor dat eutotekens verminkt werden
		"comment" => 1,
		"css_expression" => 0,
		"deny_attribute" => 0,
		"direct_nest_list" => 0,
		"elements" => "all, html, head",
		"hexdec_entity" => 0,
		"hook" => 0,
		"keep_bad" => 6,
		"lc_std_val" => 1,
		...
		"safe" => 0,
		"schemes" => "href: mailto, http, https, /; *: data, http, https, javascript",
		"style_pass" => 0,
		"tidy" => "1t2n",
		"unique_ids" => 1,
		"valid_xhtml" => 0,
	...
	private static $_generate_entities = false;
	private static $_protect_urls = false;

	/** Simpel gebruik:
	 * $processed = htmLawed::purify($text);
	 *
	 * Geavanceerd:
	 * $processed = htmLawed::purify($text, $config, $spec);
	 *
	 * @param string $t
	 * @param int $C
	 * @param array $S
	 ...
	 * @return string
	 */
	public static function purify($t, $C = 1, $S = array()) {

		$t = self::_execute($t, $C, $S);

		//correctie voor HTML-demoblokken in blog die anders dit krijgen: attribuut = "waarde":
		if (defined("ISFRAMEWORK")) {
	...
	/** Retourneer de openingstag, de inhoud en de sluittag van elementen. Deze methode werkt op basis van tekst, dus niet met cSimpleHtmlDom.
	 *
	 ...
	 * @return array
	 */
	private static function _element_get_tags_and_content($haystack, $tag) {
		// \b hieronder is cruciaal, bijv. om mixen van head en header te voorkomen:
		$success = preg_match_all("/(<{$tag}\b[^>]*>)(.*?)<\/{$tag}>/sim", $haystack, $elem);

		if ($success) {
			return Array($elem[1], $elem[2], "</{$tag}>");
		}
		else {
			return Array();
		}
	 ...
	 * @return string
	 */
	private static function _execute($t, $C = 1, $S = array()) {
		self::$_C = is_array($C) ? array_merge(self::$_C, $C) : self::$_C;

		self::_set_config($S);

		$has_head = stristr($t, "<head");
		$has_body = stristr($t, "<body");

		//DebugBreak();

		if ($has_head && $has_body) {
			preg_match("/^(.*?)<head/sim", $t, $pre);

			$head = self::_element_get_tags_and_content($has_head, "head");
			if ($head) {
				$head[1] = $head[1][0];
				$head[1] = self::_head_format($head[1]);
				$head = "\t" . $head[0][0] . "\r\n\t\t" . trim($head[1]) . "\r\n\t</head>\r\n\r\n";

				$body = self::_element_get_tags_and_content($has_body, "body");

				if ($body) {
					$body[1] = $body[1][0];
					$body[1] = self::_execute2(trim($body[1]));
					$body[1] = self::_blocklevel_spacers($body[1]);
					$body = $body[0][0] . "\r\n\t\t" . $body[1] . "\r\n\t\t</body>\r\n</html>";

					$t = preg_replace("/\r*\n\t\t<\/body>/", "\r\n\t</body>", $pre[1] . $head . $body);
				}
			}
		}

		elseif ($has_head) {
			$success = preg_match("/(^.*?<head[^>]*>)(.*?)<\/head>/sim", $t, $out);

			if ($success) {
				$out[2] = self::_head_format(trim($out[2]));

				//verwijder $out[0], zodat die niet kan zorgen voor verdubbbeling van het héle HTML-document:
				array_shift($out);
		...
		elseif ($has_body) {
			$success = preg_match("/(^.*?<body[^>]*>)(.*?)(<\/body>.+)$/sim", $t, $out);

			if ($success) {
				$out[2] = self::_execute2(trim($out[2]));

				$out[2] = self::_blocklevel_spacers($out[2]);
		...
		//indien zowel geen head als geen body gevonden (los HTML-fragment):
		else {
			$t = self::_execute2(trim($t));
		}

		//nabewerkingen:
		self::_html_corrections($t);


		return $t;
	}

	private static function _set_config($S) {
		if (!empty(self::$_C['valid_xhtml'])) {
			self::$_C['elements'] = empty(self::$_C['elements']) ? '*-acronym-big-center-dir-font-isindex-s-strike-tt' : self::$_C['elements'];
			self::$_C['make_tag_strict'] = isset(self::$_C['make_tag_strict']) ? self::$_C['make_tag_strict'] : 2;
			self::$_C['xml:lang'] = isset(self::$_C['xml:lang']) ? self::$_C['xml:lang'] : 2;
		}
		// config eles
		...
		self::$_C['elements'] =& $e;
		// config attrs
		$x = !empty(self::$_C['deny_attribute']) ? str_replace(array("\n", "\r", "\t", ' '), '', self::$_C['deny_attribute']) : '';
		$x = array_flip((isset($x[0]) && $x[0] == '*') ? explode('-', $x) : explode(',', $x . (!empty(self::$_C['safe']) ? ',on*' : '')));
		if (isset($x['on*'])) {
			unset($x['on*']);
		...
		self::$_C['deny_attribute'] = $x;
		// config URL
		$x = (isset(self::$_C['schemes'][2]) && strpos(self::$_C['schemes'], ':')) ? strtolower(self::$_C['schemes']) : 'href: aim, feed, file, ftp, gopher, http, https, irc, mailto, news, nntp, sftp, ssh, telnet; *:file, http, https';
		self::$_C['schemes'] = array();
		foreach (explode(';', str_replace(array(' ', "\t", "\r", "\n"), '', $x)) as $v) {
			list($x, $x2) = explode(':', $v, 2);
			if ($x2) {
				self::$_C['schemes'][$x] = array_flip(explode(',', $x2));
			}
		}
		if (!isset(self::$_C['schemes']['*'])) {
			self::$_C['schemes']['*'] = array('file' => 1, 'http' => 1, 'https' => 1,);
		}
		if (!empty(self::$_C['safe']) && empty(self::$_C['schemes']['style'])) {
			self::$_C['schemes']['style'] = array('!' => 1);
		}
		self::$_C['abs_url'] = isset(self::$_C['abs_url']) ? self::$_C['abs_url'] : 0;
		if (!isset(self::$_C['base_url']) or !preg_match('`^[a-zA-Z\d.+\-]+://[^/]+/(.+?/)?$`', self::$_C['base_url'])) {
			self::$_C['base_url'] = self::$_C['abs_url'] = 0;
		}
		// config rest
		self::$_C['and_mark'] = empty(self::$_C['and_mark']) ? 0 : 1;
		self::$_C['anti_link_spam'] = (isset(self::$_C['anti_link_spam']) && is_array(self::$_C['anti_link_spam']) && count(self::$_C['anti_link_spam']) == 2 && (empty(self::$_C['anti_link_spam'][0]) or self::_hl_regex(self::$_C['anti_link_spam'][0])) && (empty(self::$_C['anti_link_spam'][1]) or self::_hl_regex(self::$_C['anti_link_spam'][1]))) ? self::$_C['anti_link_spam'] : 0;
		self::$_C['anti_mail_spam'] = isset(self::$_C['anti_mail_spam']) ? self::$_C['anti_mail_spam'] : 0;
		self::$_C['balance'] = isset(self::$_C['balance']) ? (bool)self::$_C['balance'] : 1;
		self::$_C['cdata'] = isset(self::$_C['cdata']) ? self::$_C['cdata'] : (empty(self::$_C['safe']) ? 3 : 0);
		self::$_C['clean_ms_char'] = empty(self::$_C['clean_ms_char']) ? 0 : self::$_C['clean_ms_char'];
		self::$_C['comment'] = isset(self::$_C['comment']) ? self::$_C['comment'] : (empty(self::$_C['safe']) ? 3 : 0);
		self::$_C['css_expression'] = empty(self::$_C['css_expression']) ? 0 : 1;
		self::$_C['direct_list_nest'] = empty(self::$_C['direct_list_nest']) ? 0 : 1;
		self::$_C['hexdec_entity'] = isset(self::$_C['hexdec_entity']) ? self::$_C['hexdec_entity'] : 1;
		self::$_C['hook'] = (!empty(self::$_C['hook']) && method_exists("htmlLawed", self::$_C['hook'])) ? self::$_C['hook'] : 0;
		self::$_C['hook_tag'] = (!empty(self::$_C['hook_tag']) && method_exists("htmlLawed", self::$_C['hook_tag'])) ? self::$_C['hook_tag'] : 0;
		self::$_C['keep_bad'] = isset(self::$_C['keep_bad']) ? self::$_C['keep_bad'] : 6;
		self::$_C['lc_std_val'] = isset(self::$_C['lc_std_val']) ? (bool)self::$_C['lc_std_val'] : 1;
		self::$_C['make_tag_strict'] = isset(self::$_C['make_tag_strict']) ? self::$_C['make_tag_strict'] : 1;
		self::$_C['named_entity'] = isset(self::$_C['named_entity']) ? (bool)self::$_C['named_entity'] : 1;
		self::$_C['no_deprecated_attr'] = isset(self::$_C['no_deprecated_attr']) ? self::$_C['no_deprecated_attr'] : 1;
		self::$_C['parent'] = isset(self::$_C['parent'][0]) ? strtolower(self::$_C['parent']) : 'body';
		self::$_C['show_setting'] = !empty(self::$_C['show_setting']) ? self::$_C['show_setting'] : 0;
		self::$_C['style_pass'] = empty(self::$_C['style_pass']) ? 0 : 1;
		self::$_C['tidy'] = empty(self::$_C['tidy']) ? 0 : self::$_C['tidy'];
		self::$_C['unique_ids'] = isset(self::$_C['unique_ids']) && (!preg_match('`\W`', self::$_C['unique_ids'])) ? self::$_C['unique_ids'] : 1;
		self::$_C['xml:lang'] = isset(self::$_C['xml:lang']) ? self::$_C['xml:lang'] : 0;

		//Alex: onderstaande code grotendeels overbodig in class-context? :
		$GLOBALS['C'] = self::$_C;
		$S = is_array($S) ? $S : self::_hl_spec($S);
		$GLOBALS['S'] = $S;
	}

	private static function _head_format ($head) {
		$head = preg_replace("/((?:\r*\n)(\s|\t)*){1,}(?!<(?:head|\/hea|!DOC))/i", "\r\n\t\t", $head);

		return $head;
	}

	private static function _blocklevel_spacers ($t) {
		$blocklevel_regex = "(?:article|section|nav|header|footer|aside|table|div|ul|ol|dl|form|h\\d+)";

		$t = preg_replace(Array("/(\t+<{$blocklevel_regex})/i", "/(\t*<\/{$blocklevel_regex}>)/i"), Array("\r\n\\1", "\\1\r\n"), $t);
	...
	private static function _html_corrections (&$t) {

		$t = str_replace("denied:", "", $t);
		$t = preg_replace("/\t+\r*\n/", "\r\n", $t);
		$t = str_replace("\t</body>", "</body>", $t);
		$t = str_replace("><head", ">\r\n\r\n\t<head", $t);
		$t = preg_replace("/\r*\n<body/", "\r\n\t<body", $t, 1);
		$t = preg_replace("/(\r*\n){2,}\t<\/head/", "\r\n\t</head", $t, 1);

		$t = str_replace("</body>", "\t</body>", $t);
		$t = preg_replace('#(<script[^>]*>)\s+</script>#si', "\\1</script>", $t);

		$max = 50;

		$count = 0;
		while (preg_match('#\t+<img[^>]*>[ ]*<img#i', $t)) {
			$t = preg_replace('#(\t+)(<img[^>]*>)[ ]*(<img[^>]*>)#i', "\\1\\2\r\n\\1\\3", $t);
			$count++;
		...
		$count = 0;
		while (preg_match('#\t+<a.*?</a>[ ]*<img#i', $t)) {
			$t = preg_replace('#(\t+)(<a.*?</a>)[ ]*(<img[^>]*>)#i', "\\1\\2\r\n\\1\\3", $t);
			$count++;
		...
		//cHtmlLawed plaatst divs in blockquotes die geen div of p hebben. Hier verwijderen we ze weer, mits we een HTML5-element hebben (in XHTML zijn blocklevel elementen in de blockquote verplicht):
		/*if (c::o("heeftHTML5")) {
			dep::m()->replace_in_range($t, "/<blockquote>.*?<\/blockquote>/sim", "/<\/?div>/i", "");
		}*/
	}
	 ...
	 * @return mixed|string
	 */
	private static function _execute2 ($t) {
		$t = preg_replace('`[\x00-\x08\x0b-\x0c\x0e-\x1f]`', '', $t);
		if (self::$_C['clean_ms_char']) {
			...
			$x = $x + (self::$_C['clean_ms_char'] == 1 ? array("\x82" => '&#8218;', "\x84" => '&#8222;', "\x91" => '&#8216;', "\x92" => '&#8217;', "\x93" => '&#8220;', "\x94" => '&#8221;') : array("\x82" => '\'', "\x84" => '"', "\x91" => '\'', "\x92" => '\'', "\x93" => '"', "\x94" => '"'));
			$t = strtr($t, $x);
		}
		if (self::$_C['cdata'] or self::$_C['comment']) {
			$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', 'htmlLawed::hl_cmtcd', $t);
		}

		//Alex: hier html-entities omgezet:
		if (self::$_generate_entities) {
			$t = preg_replace_callback('`&amp;([a-zA-Z][a-zA-Z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', 'htmlLawed::hl_ent', str_replace('&', '&amp;', $t));
		}

		if (self::$_C['unique_ids'] && !isset($GLOBALS['hl_Ids'])) {
			$GLOBALS['hl_Ids'] = array();
		}
		if (self::$_C['hook']) {
			$method = self::$_C['hook'];
			$t = self::$$method($t, self::$_C, self::$_C);
		}
		if (self::$_C['show_setting'] && preg_match('`^[a-z][a-z0-9_]*$`i', self::$_C['show_setting'])) {
			$GLOBALS[self::$_C['show_setting']] = array('config' => self::$_C, 'spec' => self::$_C, 'time' => microtime());
		}
		// main

		//DebugBreak();
		//$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', 'htmlLawed::hl_tag', $t);
		$t = self::$_C['balance'] ? self::_hl_bal($t, self::$_C['keep_bad'], self::$_C['parent']) : $t;
		$t = ((self::$_C['cdata'] or self::$_C['comment']) && strpos($t, "\x01") !== false) ? str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05"), array('', '', '&', '<', '>'), $t) : $t;
		$t = self::$_C['tidy'] ? self::_hl_tidy($t, self::$_C['tidy'], self::$_C['parent']) : $t;
		unset($e);
	...
	private static function _hl_attrval($t, $p) {
	...
	/** Tags balanceren. Betekent dit ook dat er elementen worden toegevoegd? Of gebeurt dan in cHtmlLawed::_hl_tidy() ?
	 *
	 * @param $t
	 * @param int $do
	 * @param string $in
	 ...
	 * @return mixed|string
	 */
	private static function _hl_bal($t, $do = 1, $in = 'div') {
		// balance tags
		// by content
		...
		$t = explode('<', $t);
		$ok = $q = $p = array(); // $q seq list of open non-empty ele

		//deze geeft problemen indien output-callbacks actief zijn:
		//ob_start();
		...
		for ($i = -1, $ci = count($t); ++$i < $ci;) {
			...
			if (isset($x[0])) {
				...
				//Alex: hier soms extra div rondom element toegevoegd (daar zijn we niet blij mee)? :
				...
				elseif (strpos($x, "\x02\x04")) {
					foreach (preg_split('`(\x01\x02[^\x01\x02]+\x02\x01)`', $x, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY) as $v) {
						$collect .= (substr($v, 0, 2) == "\x01\x02" ? $v : ($do > 4 ? preg_replace('`\S`', '', $v) : ''));
					}
				}

				elseif ($do > 4) {
					$collect .= preg_replace('`\S`', '', $x);
				}
			}
			...
			list($all, $s, $e, $a, $x) = $r;
			unset($all); //puur voor foutcontrole PhpStorm toegevoegd
			// close tag
			if ($s) {
	...
	/** Let op: dit is een andere methode dan de private method htmlLawed::_hl_tidy() !!
	 * @param $var
	 *
	 * @see htmlLawed::_hl_tidy()
	 *
	 * @return string
	 */
	public static function hl_tidy_callback($var) {
		//oorspronkelijk: create_function('$m', 'return $m[1]. str_replace(array("<", ">", "\n", "\r", "\t", " "), array("\x01", "\x02", "\x03", "\x04", "\x05", "\x07"), $m[3]). $m[4];')
		return $var[1] . str_replace(array("<", ">", "\n", "\r", "\t", " "), array("\x01", "\x02", "\x03", "\x04", "\x05", "\x07"), $var[3]) . $var[4];
	}

	/** commentaren en cdata verwerken
	 *
	 * @param $t
	 *
	 * @return mixed|string
	...
	public static function hl_cmtcd($t) {
		...
		$t = $t[0];
		if (!($v = self::$_C[$n = $t[3] == '-' ? 'comment' : 'cdata'])) {
			return $t;
		}
		if ($v == 1) {
			return '';
	...
	/** Speciale tekens door htmlentities vervangen. Deze liever uitschakelen?
	 *
	 * @param $t
	 *
	 * @return string
	...
	public static function hl_ent($t) {
		...
		if ($t[0] != '#') {
			return (self::$_C['and_mark'] ? "\x06" : '&') . (isset($U[$t]) ? $t : (isset($N[$t]) ? (!self::$_C['named_entity'] ? '#' . (self::$_C['hexdec_entity'] > 1 ? 'x' . dechex($N[$t]) : $N[$t]) : $t) : 'amp;' . $t)) . ';';
		}
		if (($n = ctype_digit($t = substr($t, 1)) ? intval($t) : hexdec(substr($t, 1))) < 9 or ($n > 13 && $n < 32) or $n == 11 or $n == 12 or ($n > 126 && $n < 160 && $n != 133) or ($n > 55295 && ($n < 57344 or ($n > 64975 && $n < 64992) or $n == 65534 or $n == 65535 or $n > 1114111))) {
			return (self::$_C['and_mark'] ? "\x06" : '&') . "amp;#{$t};";
		}

		return (self::$_C['and_mark'] ? "\x06" : '&') . '#' . (((ctype_digit($t) && self::$_C['hexdec_entity'] < 2) or !self::$_C['hexdec_entity']) ? $n : 'x' . dechex($n)) . ';';
		// eof
	}

	/** Bescherming tegen ongeldige urls: die krijgen dan denied:: in plaats van het oorspronkelijke begin van de url
	 *
	 * @param $p
	 * @param null $c
	 *
	...
	public static function hl_prot($p, $c = null) {
		...
		$c = isset(self::$_C['schemes'][$c]) ? self::$_C['schemes'][$c] : self::$_C['schemes']['*'];
		static $d = 'denied:';
		...
		if (self::$_C['abs_url']) {
			if (self::$_C['abs_url'] == -1 && strpos($p, self::$_C['base_url']) === 0) { // Make url rel
				$p = substr($p, strlen(self::$_C['base_url']));
			}
			elseif (empty($m[1])) { // Make URL abs
				if (substr($p, 0, 2) == '//') {
					$p = substr(self::$_C['base_url'], 0, strpos(self::$_C['base_url'], ':') + 1) . $p;
				}
				elseif ($p[0] == '/') {
					$p = preg_replace('`(^.+?://[^/]+)(.*)`', '$1', self::$_C['base_url']) . $p;
				}
				elseif (strcspn($p, './')) {
					$p = self::$_C['base_url'] . $p;
				}
				else {
					preg_match('`^([a-zA-Z\d\-+.]+://[^/]+)(.*)`', self::$_C['base_url'], $m);
	...
	private static function _hl_regex($p) {
		// ?regex
		if (empty($p)) {
			return 0;
		}
		if ($t = ini_get('track_errors')) {
			...
			unset($o); //puur voor foutcontrole PhpStorm toegevoegd
		}
		else {
	 ...
	 * @return array
	 */
	private static function _hl_spec($t) {
		// final $spec
		$s = array();
		...
		for ($i = count(($t = explode(';', $t))); --$i >= 0;) {
			...
			foreach (explode(',', $a) as $v) {
				...
				if (isset($y[$x]['match']) && !self::_hl_regex($y[$x]['match'])) {
					unset($y[$x]['match']);
				}
				if (isset($y[$x]['nomatch']) && !self::_hl_regex($y[$x]['nomatch'])) {
					unset($y[$x]['nomatch']);
				}
			}
			if (!count($y) && !count($n)) {
	...
	public static function hl_tag($t) {
		...
		elseif (!isset(self::$_C['elements'][($e = strtolower($m[2]))])) {
			return ((self::$_C['keep_bad'] % 2) ? str_replace(array('<', '>'), array('&lt;', '&gt;'), $t) : '');
		}
		...
		if (self::$_C['make_tag_strict'] && isset($eD[$e])) {
			$trt = self::_hl_tag2($e, $a, self::$_C['make_tag_strict']);
			if (!$e) {
				return ((self::$_C['keep_bad'] % 2) ? str_replace(array('<', '>'), array('&lt;', '&gt;'), $t) : '');
			}
		}
		...
		// clean attrs
		$rl = isset(self::$_C[$e]) ? self::$_C[$e] : array();
		$a = array();
		$nfr = 0;
		foreach ($aA as $k => $v) {
			if (((isset(self::$_C['deny_attribute']['*']) ? isset(self::$_C['deny_attribute'][$k]) : !isset(self::$_C['deny_attribute'][$k])) && (isset($aN[$k][$e]) or isset($aNU[$k]) or preg_match('`data-((?!xml)[^:]+$)`', $k)) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) {
				if (isset($aNE[$k])) {
					$v = $k;
				}
				...
				//numerieke entities omzetten? :
				if ($k == 'style' && !self::$_C['style_pass']) {
					if (false !== strpos($v, '&#')) {
					...
					if (self::$_protect_urls) {
						$v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', 'htmlLawed::hl_prot', $v);
					}

					$v = !self::$_C['css_expression'] ? preg_replace('`expression`i', ' ', preg_replace('`\\\\\S|(/|(%2f))(\*|(%2a))`i', ' ', $v)) : $v;
				}
				...
				elseif (isset($aNP[$k]) or strpos($k, 'src') !== false or $k[0] == 'o') {
					...
					if ($k == 'href') { // X-spam
						if (self::$_C['anti_mail_spam'] && strpos($v, 'mailto:') === 0) {
							$v = str_replace('@', htmlspecialchars(self::$_C['anti_mail_spam']), $v);
						}
						elseif (self::$_C['anti_link_spam']) {
							$r1 = self::$_C['anti_link_spam'][1];
							if (!empty($r1) && preg_match($r1, $v)) {
								continue;
							}
							$r0 = self::$_C['anti_link_spam'][0];
				...
				if (isset($rl[$k]) && is_array($rl[$k]) && ($v = self::_hl_attrval($v, $rl[$k])) === 0) {
					continue;
				}
				$a[$k] = str_replace('"', '&quot;', $v);
			}
		...
		if ($depTr) {
			...
			foreach ($a as $k => $v) {
				...
				elseif ($k == 'name') {
					if (self::$_C['no_deprecated_attr'] == 2 or ($e != 'a' && $e != 'map')) {
						unset($a['name']);
					}
		...
		// unique ID
		if (self::$_C['unique_ids'] && isset($a['id'])) {
			if (preg_match('`\s`', ($id = $a['id'])) or (isset($GLOBALS['hl_Ids'][$id]) && self::$_C['unique_ids'] == 1)) {
				unset($a['id']);
			}
			else {
				while (isset($GLOBALS['hl_Ids'][$id])) {
					$id = self::$_C['unique_ids'] . $id;
				}
				$GLOBALS['hl_Ids'][($a['id'] = $id)] = 1;
			}
		}
		// xml:lang
		if (self::$_C['xml:lang'] && isset($a['lang'])) {
			$a['xml:lang'] = isset($a['xml:lang']) ? $a['xml:lang'] : $a['lang'];
			if (self::$_C['xml:lang'] == 2) {
				unset($a['lang']);
			}
		}
		// for transformed tag
		...
		if (empty(self::$_C['hook_tag'])) {
			...
			$method = self::$_C['hook_tag'];
			return self::$$method($e, $a);
		}
		// eof
	}

	private static function _hl_tag2(&$e, &$a, $t = 1) {
		// transform tag
		if ($e == 'big') {
			$e = 'span';
	 ...
	 * @return mixed
	 */
	private static function _hl_tidy($t, $w, $p) {
		// Tidy/compact HTM
		if (strpos(' pre,script,textarea', "$p,")) {
			return $t;
		}
		$t = preg_replace(array('`(<\w[^>]*(?<!/)>)\s+`', '`\s+`', '`(<\w[^>]*(?<!/)>) `'), array(' $1', ' ', '$1'), preg_replace_callback(array('`(<(!\[CDATA\[))(.+?)(\]\]>)`sm', '`(<(!--))(.+?)(-->)`sm', '`(<(pre|script|textarea)[^>]*?>)(.+?)(</\2>)`sm'), 'htmlLawed::hl_tidy_callback', $t));
		if (($w = strtolower($w)) == -1) {
		...
		while ($X) {
			...
			//ob_start();
			if (isset($d[$p])) {
				$collect .= str_repeat($s, ++$n);
			}
			$collect .= ltrim(array_shift($t));
